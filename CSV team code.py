...
async def main():
    ...
    # Turn 180
    await motors.move_for_degrees(motor_pair.PAIR_1,245,100, velocity= 50)
    # Go to catch the can
    ...
    await motor.run_for_degrees(port.F, 295, 100)
    #Back to silver bar
    ...
    await motors.move_for_degrees(motor_pair.PAIR_1,300,0, velocity= speed)
...
#this finds the reflected colour of the sensors
    ...
    if lgreen is color.GREEN or rgreen is color.GREEN:# this move me left or right
        if lgreen is color.GREEN: # if the left sensor discovers green then...
                #await motors.move_for_degrees(motors.PAIR_1,3600,0, velocity = 10)
            await motors.move_for_degrees(motors.PAIR_1,310,-45, velocity = speed)
    ...
    # this calculates the angle to go forward
        motors.move(motors.PAIR_1, (lcolour - rcolour) *2, velocity = speed)


async def towll():
    await motor_pair.move_for_degrees(motors.PAIR_1,30,0, velocity = 0 - speed)
